...
@section('content')
    <section class="py-16 px-32">
        ...
        <div class="flex justify-start my-5">
            <div class="w-1/2">
                ...
                <div class="flex items-center justify-start my-4">
                    ...
                    <h3 class="text-xl font-semibold text-slate-800">{{ json_decode($gor->address)->detail }}</h3>
                </div>
                <h1 class="text-2xl font-bold text-slate-800 my-2">Contact</h1>
                <p class="text-xl font-medium text-slate-800 mb-4">{{ $gor->user->name }}</p>
                <h1 class="text-2xl font-bold text-slate-800 my-2">Jam Operasional</h1>
                <p class="text-xl font-medium text-slate-800 my-2">{{ date('H:i', strtotime($gor->opening_hour)) }} - {{ date('H:i', strtotime($gor->closing_hour)) }} WIB</p>
            </div>
            <div class="w-1/2">
                ...
                @foreach ($gor->field as $field)
                        <div class="flex items-center justify-between mb-3">
                            ...
                            <p class="text-lg text-sky-900 font-medium">Rp. {{ number_format($field->price, 2, ',', '.') }}/Jam</p>
                        </div>
                @endforeach
                <div class="flex items-center justify-start mt-4">
                    <p class="text-xl font-semibold text-slate-800 mr-3">Rating</p>
        ...
        <a href="/sporthall/{{ $gor->slug }}/order" class="text-lg font-bold text-white bg-sky-800 py-3 px-24 rounded-2xl hover:shadow-lg hover:opacity-50 transition duration-300 ease-in-out">ORDER</a>
        </div>
    </section>
@endsection