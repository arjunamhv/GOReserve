...
@section('content')
    <section class="py-16 px-12 lg:px-32">
        ...
        <div class="block lg:flex justify-start my-5">
            <div class="lg:w-1/2">
                <h1 class="text-2xl lg:text-3xl font-bold text-slate-800">{{ $gor->name }}</h1>
                ...
                <h1 class="text-xl lg:text-2xl font-bold text-slate-800 my-2">Contact</h1>
                <p class="text-base lg:text-lg font-medium text-slate-800 mb-4">{{ $gor->user->name }}</p>
                <h1 class="text-xl lg:text-2xl font-bold text-slate-800 my-2">Jam Operasional</h1>
                <p class="text-base lg:text-lg font-medium text-slate-800 my-2">{{ date('H:i', strtotime($gor->opening_hour)) }} - {{ date('H:i', strtotime($gor->closing_hour)) }} WIB</p>
            </div>
            <div class="lg:w-1/2">
                ...
                @foreach ($gor->field as $field)
                        <div class="flex items-center justify-between mb-2">
                            <div class="flex items-center justify-start">
                                ...
                                <p class="text-sm lg:text-lg font-medium text-slate-800">{{ $field->name }}</p>
                            </div>
                            <p class="text-sm lg:text-base text-sky-900 font-medium">Rp. {{ number_format($field->price, 2, ',', '.') }}/Jam</p>
                        </div>
                @endforeach
                <div class="flex items-center justify-start mt-4">
                    <p class="text-xl lg:text-2xl font-bold text-slate-800 mr-3">Rating</p>
        ...
        <div class="pt-5 border-t border-slate-700 flex justify-end items-center">
            <a href="/sporthall/{{ $gor->slug }}/check" class="text-md font-semibold text-sky-800 bg-white border border-sky-800 px-12 rounded-2xl py-3 lg:px-24 hover:shadow-lg hover:opacity-50 transition duration-300 ease-in-out">CEK JADWAL</a>
            <a href="/sporthall/{{ $gor->slug }}/order" class="text-md font-semibold text-white bg-sky-800 rounded-2xl ml-12 py-3 px-24 hover:shadow-lg hover:opacity-50 transition duration-300 ease-in-out">ORDER</a>
        </div>
    </section>
@endsection