...
@section('content')
    ...
    @endif
        <div class="w-4/5 mx-auto lg:w-full lg:mx-0 rounded-3xl bg-slate-300 p-4 mb-10">
            <h1 class="text-center text-2xl lg:text-3xl font-bold text-slate-800 mb-4">{{ $gor->name }}</h1>
            <div class="block lg:flex items-center justify-between">
                <img src="{{ asset('../img/login-image.png') }}" alt="lapang" class="w-full h-[250px] mb-2 lg:mb-0 lg:w-2/5 lg:h-[400px] rounded-xl">
                <div class="lg:w-1/2">
                    <form method="POST" action="{{ route('store', ['gor' => $gor->slug]) }}">
                        ...
                        <div class="flex justify-between items-center">
                            <div class="">
                                ...
                                <input type="date" name="booking_date" id="booking_date" class="bg-slate-200 text- p-1 lg:p-2 rounded-md focus:outline-none focus:ring-primary focus-ring1 focus:border-primary" required>
                            </div>
                            <div class="">
                                <p class="text-base font-bold text-slate-800">
                                    Jam
                                </p>
                                <input type="time" name="start_time" id="start_time" class="bg-slate-200 text-dark p-1 lg:p-2 rounded-md focus:outline-none focus:ring-primary focus-ring1 focus:border-primary" required>
                            </div>
                            <div class="">
                                <p class="text-base font-bold text-slate-800">
                                    Durasi
                                </p>
                                <input type="number" name="duration" id="duration" min="1" max="12" class="bg-slate-200 text-dark p-1 lg:p-2 rounded-md focus:outline-none focus:ring-primary focus-ring1 focus:border-primary" required>
                            </div>
                        </div>
            ...
            <form action="{{ route('transaction', ['gor' => $gor->slug]) }}" method="POST">
                ...
                <div class="w-full border border-slate-800 rounded-lg p-4">
                    ...
                    <p class="text-sky-800 text-lg font-semibold mb-3">{{ $formData['username'] }}</p>
                    <p class="text-sky-800 text-lg font-semibold mb-3">{{ $formData['field_name'] }}</p>
                    <div class="flex justify-between items-center mb-3">
                        <p class="text-sky-800 text-lg font-semibold">Pukul {{ $formData['start_time'] }} - {{ $formData['end_time'] }}</p>
                        <p class="text-sky-800 text-xl font-semibold">Rp. {{ $formData['price'] }}</p>
                    </div>
                    <div class="w-full border-t border-slate-700 flex justify-between items-center py-3 mb-3">
                        <p class="text-sky-800 text-lg font-bold">TOTAL</p>
                        <p class="text-sky-800 text-xl font-bold">Rp. {{ number_format($formData['price'], 2, ',', '.') }}</p>
                    </div>
